...
(defun sly-db-exit (thread _level &optional stepping)
  ...
  (when-let (sly-db (sly-db-find-buffer thread))
...
(defun sly-db-fetch-all-frames ()
  (let ((inhibit-read-only t)
    ...
    (let ((last (sly-db-frame-number-at-point)))
      ...
      (save-excursion
        (sly-db-insert-frames (sly-eval `(swank:backtrace ,(1+ last) nil))
                            nil)))))
...
(defun sly-db-disassemble (frame-number)
  ...
  (interactive (list (sly-db-frame-number-at-point)))
  (sly-eval-async `(swank:sly-db-disassemble ,frame-number)
    (lambda (result)
...
(defun sly-db-abort ()
  ...
  (interactive)
  (sly-eval-async '(swank:sly-db-abort)
...
Interactively get the number from a button at point."
  ...
  (sly-rex ()
      ((list 'swank:invoke-nth-restart-for-emacs sly-db-level restart-number))
...
(defun sly-db-break-with-default-debugger (&optional dont-unwind)
  ...
  (interactive "P")
  (sly-rex ()
      ((list 'swank:sly-db-break-with-default-debugger
             (not (not dont-unwind)))
...
(defun sly-db-step (frame-number)
  ...
  (interactive (list (sly-db-frame-number-at-point)))
  (sly-eval-async `(swank:sly-db-step ,frame-number)))

(defun sly-db-next (frame-number)
  "Step over call."
  (interactive (list (sly-db-frame-number-at-point)))
  (sly-eval-async `(swank:sly-db-next ,frame-number)))

(defun sly-db-out (frame-number)
  "Resume stepping after returning from this function."
  (interactive (list (sly-db-frame-number-at-point)))
  (sly-eval-async `(swank:sly-db-out ,frame-number)))
...
The debugger is entered when the frame exits."
  (interactive (list (sly-db-frame-number-at-point)))
  (sly-eval-async `(swank:sly-db-break-on-return ,frame-number)
...
(defun sly-db-break (name)
  ...
  (sly-eval-async `(swank:sly-db-break ,name)
    (lambda (msg) (sly-message "%s" msg))))
...
return that value, evaluated in the context of the frame."
  ...
  (sly-rex ()
      ((list 'swank:sly-db-return-from-frame frame-number string))
...
(easy-menu-define sly-sly-db-menu sly-db-mode-map "SLDB Menu"
  (let ((C '(sly-connected-p)))
    `("SLDB"
